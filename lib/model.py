...
class Ganomaly(BaseModel):
    ...
    def __init__(self, opt, dataloader):
        ...
        self.l_adv = l2_loss #对抗损失 mean((x - y) ** 2)
        self.l_con = nn.L1Loss() #重建一致性损失|G(input) - input|
        self.l_enc = l2_loss #编码器一致性损失
        self.l_bce = nn.BCELoss() #二元交叉熵损失
    ...
    def forward_g(self):
        ...
        """
        self.fake, self.latent_i, self.latent_o = self.netg(self.input) #decoder,encoder1,encoder2
    ...
    def backward_g(self):
        ...
        """
        self.err_g_adv = self.l_adv(self.netd(self.input)[1], self.netd(self.fake)[1]) #原始图像和生成图像特征
        self.err_g_con = self.l_con(self.fake, self.input) #原始图像和生成图像像素
        self.err_g_enc = self.l_enc(self.latent_o, self.latent_i) #输入和重建图像的潜向量
